...
func TestBasicAgree(t *testing.T) {
	servers := 5
	cfg := make_config(t, servers, false) //初始化
	defer cfg.cleanup()
...
loop:
	for try := 0; try < 5; try++ {
		...
		for j := 0; j < servers; j++ {
			if ct, _ := cfg.rafts[j].GetState(); ct != term {
				// 任期又变了 -- 无法计算这个任期处理的信息数量了
				continue loop
			}
		}
		...
		cmds := []int{}
		for index := range is {
			cmd := cfg.wait(index, servers, term) //等待大家都提交这些cmd
			if ix, ok := cmd.(int); ok {
				if ix == -1 {
		...
		for ii := 0; ii < iters; ii++ { //看看是不是所有的cmd都成功提交上去了,且cmd和索引号是对应的
			x := 100 + ii
			ok := false
	...
	if !success { //不成功必然是因为中间换届了,之前领导想提交的没成功
